<x-app-layout>
    ...
    <div class="card-body">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form method="post" action="{{ route('payment-methods.update', ['paymentMethod' => $paymentMethod]) }}" class="mt-6 space-y-6">
                    @csrf
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="mb-3">
                                <x-input-label for="name" :value="__('Name')" />
                                <x-text-input id="name" name="name" type="text" class="form-control" :value="$paymentMethod->name" required autofocus autocomplete="name" />
                        ...
                        <div class="col-lg-2">
                            ...
                            <select id="number_max_installments" name="number_max_installments" class="form-control">
                                ...
                                @for ($i = 1; $i <= App\Models\PaymentMethod::MAX_INSTALLMENTS ; $i++)
                                    <option value="{{ $i }}" {{ $paymentMethod->number_max_installments == $i ? 'selected' : '' }}> {{ $i }} </option>
                                    @endfor
                            </select>
                        ...
                        <div class="col-lg-2">
                            ...
                            <select id="show_on_store" name="show_on_store" class="form-control">
                                <option value=""> Selecione </option>
                                <option value="0" {{ $paymentMethod->show_on_store == 0 ? 'selected' : '' }}> {{ __('No') }} </option>
                                <option value="1" {{ $paymentMethod->show_on_store == 1 ? 'selected' : '' }}> {{ __('Yes') }} </option>
                        ...
                        <div class="col-lg-2">
                            ...
                            <select id="show_on_finance" name="show_on_finance" class="form-control">
                                <option value=""> Selecione </option>
                                <option value="0" {{ $paymentMethod->show_on_finance == 0 ? 'selected' : '' }}> {{ __('No') }} </option>
                                <option value="1" {{ $paymentMethod->show_on_finance == 1 ? 'selected' : '' }}> {{ __('Yes') }} </option>
